...
class CompraAgilApiService
{
    ...
    private function opcionesHttpMp(?float $deadlineMicrotime = null): array
    {
        ...
        $lowSpeedLimitBytes = max(0, (int) config('cotiz.mercadopublico.api_low_speed_limit_bytes', 10));
        ...
        if ($lowSpeedLimitBytes > 0) {
            $lowSpeedTimeSeg = min(
                max(5, (int) config('cotiz.mercadopublico.api_low_speed_time_segundos', 20)),
                max(5, $timeoutSeg - 5),
            );
            $curlOpts[CURLOPT_LOW_SPEED_TIME] = $lowSpeedTimeSeg;
            $curlOpts[CURLOPT_LOW_SPEED_LIMIT] = $lowSpeedLimitBytes;
        }
